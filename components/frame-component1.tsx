...
const FrameComponent1: NextPage<FrameComponent1Type> = ({ className = "" }) => {
  ...
  const handleSwapClick = async () => {
    try {
      ...
      // Replace with your recipient wallet address
      const recipient = new PublicKey("TuDireccionDeBilleteraAqui");
      ...
      // Create a transaction to send 0.01 SOL
      ...
      setStatusMessage("Transaction confirmed! Tokens will be sent back shortly.");
    } catch (error) {
      ...
      <div className="self-stretch w-[1224px] flex flex-row items-end justify-start gap-[71px] max-w-full mq1025:flex-wrap">
        <div className="self-stretch flex-1 rounded-21xl flex flex-col items-start justify-start py-10 px-[39px] box-border gap-10 bg-[url('/swap-module@3x.png')] bg-cover bg-no-repeat bg-[top] max-w-full z-[2] mq450:min-w-full mq1025:min-h-[auto]">
          ...
          <button
            ...
            className="cursor-pointer [border:none] p-1 bg-deepskyblue self-stretch rounded-25xl flex flex-row items-center justify-center relative"
          >
          ...
          {transactionHash && (
            <div className="mt-4 text-center text-deepskyblue">
              Transaction Hash: {transactionHash}
            </div>
          )}
          {statusMessage && (
            <div className="mt-2 text-center text-light-gray">
              {statusMessage}
            </div>
          )}
        </div>
